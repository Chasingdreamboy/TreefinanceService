...
typedef NS_ENUM(NSInteger, JSBridgeCodeType)
{
    kJSBridgeCodeUnkown = -1199,                //未知错误
    kJSBridgeCodeInternalErr = -1200,           //内部错误
    kJSBridgeCodeSuccess = 100,                 //调用且逻辑处理成功
    kJSBridgeCodeUnLogin = -1100,               //用户未登录（一般发生在接口需要用户处于登录状态，而当前用户未登录）
    kJSBridgeCodeErrModule = -1101,             //模块未找到（一般发生在H5的模块输入错误的时候，请检查相关的输入是否正确）
    kJSBridgeCodeErrMethod = -1102,             //方法未找到（一般发生在H5的方法输入错误的时候，请检查相关的输入是否正确）
    kJSBridgeCodeErrParams = -1103,             //参数错误（一般发生H5带入的参数与接口需要的参数不致的时候，请检查参数是否正确）
};
...
@property (nonatomic, assign) TBSWebViewController *parentVC;
...
// *  初始构造JSBridge
// *
// *  @param webView              内嵌的浏览器
// *  @param parentVC             承载的父级VC
// *  @param webViewDelegate      浏览器的代理
// *  @param bundle               文件路径
// *  @param handler              初始的H5回应处理
// */
...
//                         bundle:(NSBundle*)bundle handler:(JSBHandler)handler;
//customer
