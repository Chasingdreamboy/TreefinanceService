...
//- (instancetype)initWithWebView:(UIWebView*)webView parentVC:(UIViewController *)parentVC
- (instancetype)initWithParentVC:(TBSWebViewController *)parentVC {
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    ...
    if(!(_numberOfUrlRequests > 0)) {
        if(![[webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"typeof (%@) == 'object'",JS_BRIDGE]] isEqualToString:@"true"]) {
            NSBundle *resourceBundle = [TBSHelper bundle];
            NSBundle *bundle = _resourceBundle ?: resourceBundle;
            NSString *filePath = [bundle pathForResource:JS_BRIDGE_FILE_NAME ofType:@"js"];
            NSLog(@"error filePath = %@",filePath );
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if(webView != _jsWebView) return;
    _numberOfUrlRequests--;
//    NSLog(@"finish AAA== %ld", _numberOfUrlRequests);
 ...
 navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([[url scheme] isEqualToString:JSBRIDGE_URL_SCHEME]) {
        if ([[url host] isEqualToString:JSBRIDGE_URL_MESSAGE]) {
            ...
            NSLog(@"shouldStartLoadWithRequest: WARNING: Received unknown command %@",url);
        }
        return NO;
    }
    return YES;
...
+ (void)callCallbackForWebView:(UIWebView *)webView params:(NSDictionary *)params response:(id)response code:(DSOperationState)code {
    if(params) {
        ...
        if(callbackID) {
            ...
            NSDictionary *retObj = [JSBridge getReturnObjectWithCode:code data:response];
...
+ (void)callEventCallback:(JSBResponseCallback)responseCallback data:(id)data {
    if(responseCallback != nil) {
        responseCallback([JSBridge getReturnObjectWithCode:DSOperationStateSuccess data:data]);
    }
}
...
+ (NSDictionary *)getReturnObjectWithCode:(DSOperationState)code data:(id)data {
    ...
    if (code == DSOperationStateSuccess) {
        ...
        NSString *errorMsg = [JSBridge getErrMsgWithCode:code];
...
- (void)dispatchMessage:(NSDictionary *)message {
    ...
    NSLog(@"JSB Action: SEND: %@",messageJSON);
    ...
    if(nativeModule == nil) {
        ...
        if(objClass) {
            @try {
                ...
                NSLog(@"getNativeModuleFromName: EXCEPTION: %@", name);
                nativeModule = nil;
            } @finally {
            }
        } else {
            NSLog(@"Unsupported Module: %@", name);
        }
    }
    return nativeModule;
}
...
- (void)handleReturnValue:(NSInvocation *)invoker sig:(NSMethodSignature *)sig webView:(UIWebView *)webView apiName:(NSString *)apiName status:(BOOL)status {
    ...
    if((invoker != nil) && (sig != nil)) {
        if([sig methodReturnLength] > 0) {
            ...
            if(retValue) {
                NSLog(@"handleReturnValue:%@", retValue);
...
- (void)processEventHandler:(UIWebView *)webView message:(NSDictionary *)message responseCallback:(JSBResponseCallback)responseCallback {
    ...
    if(eventName) {
        ...
        if(!handler) {
            @try {
                ...
                if(jsModule) {
                    ...
                    NSLog(@"processEventHandler: EXCEPTION: No Plugin: %@", eventName);
                }
            } @catch (NSException *exception) {
                NSLog(@"processEventHandler: EXCEPTION: %@", eventName);
                handler = nil;
            } @finally {
            }
        }
        ...
        if(_bridgeHandler) {
            ...
            NSLog(@"EXCEPTION: No handler for message from JS: %@", message);
        }
    }
}
...
- (void)processJSEventQueue:(UIWebView *)webView {
    ...
    if(![messages isKindOfClass:[NSArray class]]) {
        NSLog(@"flushMessageQueue: WARNING: Invalid %@ received: %@", [messages class], messages);
        return;
    }
    for (NSDictionary *message in messages) {
        if (![message isKindOfClass:[NSDictionary  class]]) {
            NSLog(@"flushMessageQueue: WARNING: Invalid %@ received: %@", [message class], message);
            continue;
        }
        NSLog(@"flushMessageQueue: RCVD: %@", message);
    ...
    @try {
        ...
        if(jsModule) {
            ...
            if (sig) {
                ...
                [JSBridge callCallbackForWebView:webView params:params response:nil code:DSOperationStatePluginErrorMethod];
            }
        } else {
            [JSBridge callCallbackForWebView:webView params:params response:nil code:DSOperationStatePluginErrorModule];
        }
    } @catch (NSException *exception) {
        NSLog(@"processJSAPIRequest: EXCEPTION: %@",exception);
        [JSBridge callCallbackForWebView:webView params:params response:nil code:DSOperationStateUnknownError];
    } @finally {
    }
}
